  ...
  <div class="col-xl-12 col-md-12 col-sm-12 pagination-header">
    ...
    <div class="numb-per-page btn-no-outline input-group-pagination align-items-center" style="justify-content: flex-end">
      ...
      <select v-model="perPage" class="form-select btn-no-outline select-css focus-no-outline" aria-label="Default select example">
        <option v-for="option in perpageOptions" :key="option" class="dropdown-item text-center" :value="option">
          {{ option }}
        </option>
      </select>
...
export default {
    data(){
        return {
            ...
            perPage: 10    //mac dinh 10 ban ghi 1 trang
        }
    },
    props: {
        pagination: {
    ...
    watch:{
        //theo doi state perPage - khi thay doi se goi method hanleChangePerPage de render lai view
        perPage(_perpage){
            this.handleChangePerPage(_perpage)
        }
    },
    methods: {
